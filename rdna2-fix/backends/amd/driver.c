...
static void _launch(int gridX, int gridY, int gridZ, int num_warps,
  ...
  if (gridX * gridY * gridZ == 0)
    return;
  if (num_ctas > 1) {
    // hipDrvLaunchKernelEx / HIP_LAUNCH_CONFIG are not available in ROCm 7.1
    // headers and RDNA2 (gfx1030) does not support multi-CTA cluster launches.
    PyErr_SetString(PyExc_RuntimeError,
                    "num_ctas > 1 is not supported on this HIP runtime");
    return;
...
static PyObject *launchKernel(PyObject *self, PyObject *args) {
  ...
  // triton-tiger packed_metadata is a 6-tuple: (num_warps, num_ctas, shared,
  ...
  if (!PyArg_ParseTuple(args, "piiiKK(iiiiii)OOOiOy*O", &launch_cooperative_grid,
                        &gridX, &gridY, &gridZ, &_stream, &_function,
                        &num_warps, &num_ctas, &shared_memory,
                        &clusterDimX, &clusterDimY, &clusterDimZ,
                        &launch_metadata, &launch_enter_hook,
  ...
  // Number of parameters passed to kernel. + 1 for global scratch.
  // triton-tiger kernels do not have a profile scratch parameter.
  ...
  // triton-tiger kernels expect only global_scratch, not profile_scratch.
